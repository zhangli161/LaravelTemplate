...
class CouponController extends Controller
{
    ...
    protected function detail($id)
    {
        ...
        $show->distribute_methods('发放方式', function (Grid $grid) {
            $grid->method('发放方式')->using([1 => '积分兑换', 2 => "发放领取"]);
            $grid->price('价格');
...
//			});
            ...
            $form->hasMany('distribute_methods', '发放方式', function (Form\NestedForm $form) {
                $form->select('method', '发放方式')->options([1 => '积分兑换', 2 => "活动领取"])->default(1);
                $form->decimal('price', '价格')->help("发放方式为【活动领取】时无效")->default(0);
//                $form->switch('send_message', '发送系统消息')->help("发放方式为【活动领取】时无效");
...
//                $form->datetimeRange("attr.start_date", "attr.end_date",'活动时间');

                $form->embeds('attr', '活动领取-附加信息', function ($form) {
                    $form->image("image", "活动图片")->help("发放方式为【活动领取】时生效");
                    $form->datetimeRange("start_date", "end_date", '活动时间')->rules('required|after:now');
                    $form->editor('content', "活动内容")->help("发放方式为【活动领取】时生效");
