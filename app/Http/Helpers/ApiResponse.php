...
class ApiResponse
{
     ...
     */
    public static function makeResponse($result, $ret, $code = null, $mapping_function = null, $params = [])
    {
        if ($code == null) {
            if ($result)
                $code = self::SUCCESS_CODE;
            else
                $code = self::UNKNOW_ERROR;
        }
        $rsp = [];
        $rsp['code'] = $code;
...
//		return response()->json($rsp, 200);
    ...
    {
        return self::makeResponse(false, '缺少参数', self::MISSING_PARAM);
    }
}